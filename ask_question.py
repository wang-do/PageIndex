...
_INSTRUCTION_ZH = (
    ...
    "输出格式（硬性要求）：回答必须输出为连续的 JSON 对象序列，每个对象两个字段："
    "t（该段正文，可含 markdown 粗体 ** **、列表标记 - 与换行 \\n）和 c（该段依据的"
    "条款号数组，如 [\"GB55037-2022 6.4.1\"]，无依据则空数组）。对象之间直接相连，"
    "不要逗号、不要换行分隔、不要外层数组。示例："
    "{\"t\":\"防火门应能自行关闭。\",\"c\":[\"GB55037-2022 6.4.1\"]}"
    "{\"t\":\"**甲级防火门**用于：\\n- 防火墙上的门\",\"c\":[\"GB55037-2022 6.4.2\"]}"
    "条款号必须从检索到的条文中取，禁止编造。反问澄清时输出单个 t 对象、c 为空数组。"
